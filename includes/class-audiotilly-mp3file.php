...
//        return sprintf('%d:%02d', $duration / 60, $duration % 60);
    ...
    public static function formatTimeT0H0M0S($duration) // as ThHmMsS
    {
        if (is_null($duration)) {
            return '';
        }
//        return sprintf('%d:%02d', $duration / 60, $duration % 60);
        ...
        $minutes = floor( ($duration - $hours * 3600) / 60);
        $seconds = $duration - $hours * 3600 - $minutes * 60;
        return sprintf('T%dH%dM%dS', $hours, $minutes, $seconds);
    }
    ...
    public function getDuration($use_cbr_estimate = false)
    {
        ...
        while (!feof($fd))
        {
            ...
            if (strlen($block) < 10) {
                ...
                $info = static::parseFrameHeader(substr($block, 0, 4));
                ...
                fseek($fd, 118, SEEK_CUR); // skip over id3v1 tag size
            } else {
                fseek($fd, -9, SEEK_CUR);
            }
            if ($use_cbr_estimate && !empty($info['Bitrate'])) {
